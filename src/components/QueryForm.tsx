...
export const QueryForm: React.FC<QueryFormProps> = ({
  ...
  const handleParseQuickPhrase = () => {
    if (!quickPhrase.trim()) return;
    const text = quickPhrase.trim();

    // Regex extraction for year (4 digits 19xx or 20xx)
    const yearMatch = text.match(/\b(19\d{2}|20\d{2})\b/);
    const extractedYear = yearMatch ? yearMatch[1] : '';

    // Regex extraction for common engines (e.g., 1.0, 1.4, 1.6, 2.0, 8v, 16v)
    const engineMatch = text.match(/\b([1-3]\.[0-9](\s*(?:8v|16v|flex|gasolina|tsi|fire|e-torq|sigma|ea111))?)\b/i);
    const extractedEngine = engineMatch ? engineMatch[0] : '';

    let cleaned = text;
    if (extractedYear) cleaned = cleaned.replace(extractedYear, '');
    if (extractedEngine) cleaned = cleaned.replace(extractedEngine, '');

    // Check if common parts match
    let detectedPart = '';
    for (const cp of COMMON_PARTS) {
      if (cleaned.toLowerCase().includes(cp.toLowerCase())) {
        detectedPart = cp;
        cleaned = cleaned.replace(new RegExp(cp, 'gi'), '');
        break;
      }
    }

    if (!detectedPart) {
      // heuristic: first words or remainder
      const words = cleaned.trim().split(/\s+/);
      if (words.length >= 2) {
        detectedPart = words.slice(0, 2).join(' ');
        cleaned = words.slice(2).join(' ');
      } else {
        detectedPart = cleaned.trim();
        cleaned = '';
      }
    }

    const detectedVehicle = cleaned.replace(/ano|motor|mod|modelo|com|sem/gi, '').trim();

    if (detectedPart) setPart(detectedPart);
    if (detectedVehicle) setVehicle(detectedVehicle);
    if (extractedYear) setYear(extractedYear);
    if (extractedEngine) setEngine(extractedEngine);
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 mb-4 border-b border-slate-100">
        ...
        <button
          ...
          className="text-xs font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-2.5 py-1.5 rounded-md flex items-center gap-1.5 transition-colors self-start sm:self-auto"
        >
          <Zap className="w-3.5 h-3.5 text-amber-500" />
          {showPhraseInput ? 'Ocultar Frase Livre' : 'Colar Pedido do Cliente (Voz/Texto)'}
        </button>
      </div>

      {/* Quick Phrase Auto-Extractor Box */}
      {showPhraseInput && (
        <div className="mb-5 p-3.5 rounded-lg bg-slate-50 border border-blue-200 animate-fadeIn">
          <label className="block text-xs font-semibold text-slate-700 mb-1.5">
            Texto falado pelo cliente no balcão ou WhatsApp:
          </label>
          <div className="flex gap-2">
            <input
              ...
              placeholder="Ex: amortecedor dianteiro Onix 2015 1.4 com freio ABS"
              className="flex-1 text-xs px-3 py-2 bg-white border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
                  handleParseQuickPhrase();
                }
              }}
            />
            <button
              type="button"
              onClick={handleParseQuickPhrase}
              className="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-xs font-semibold flex items-center gap-1 shrink-0"
            >
